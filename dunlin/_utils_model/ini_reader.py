...
def parse_section(name, config, append_model_name=False):
    ...
    for key in data:
        ...
        #Parameter bounds
        if key == 'param_bounds':
            section_data['param_bounds'] = parse_bounds(name, config, 'param_bounds', append_model_name)
...
@wrap_try('bounds')
def parse_bounds(name, config, key='param_bounds', append_model_name=False):
    data = config[name]
    
    bounds_dict = parse_params(data[key])
