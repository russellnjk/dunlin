...
class IntResult():
    ...
    def __init__(self, model, t, y, p, scenario, _exv_names=None):
        p_table = self.tabulate_params(model, t, y, p, scenario)
        p_dict  = dict(zip(model._params, p_table))
        
        exv_results = {}
        exv_names   = model._exvs if _exv_names is None else _exv_names
        for exv_name in exv_names:
            exv                   = model._exvs[exv_name]
            exv_results[exv_name] = self.evaluate_exv(exv, t, y, p_table)
        
        self.model_key = model.model_key
        self._sim      = {**model._sim(t, y, p), **p_dict, **exv_results}
        self._t        = t
