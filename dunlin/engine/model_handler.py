...
class Model():
    ...
    #Constructors
    ###############################################################################
    def __init__(self, name, states, params, inputs, equations, meta=None, tspan=None, int_args=None, modify=None, solver_args=None):
        ...
        set_attr('meta',        meta)
        set_attr('solver_args', solver_args)
        ...
        if type(equations) == str:
            ...
            set_attr('code',        code)
            set_attr('func',        func)
        elif type(equations).__name__ == 'function':
            set_attr('code',        code)
            set_attr('func',        func)
        else:
    ...
    def __setattr__(self, attr, value):
        if attr == 'init_vals':
            ...
            super().__setattr__(attr, value_[self.states])
            ...
            super().__setattr__(attr, value_[self.params])
            ...
            super().__setattr__(attr, value_[self.inputs])
        elif attr == 'tspan':
...
if __name__ == '__main__':    
    ...
    assert all( np.isclose(tspan[1], np.linspace(300, 1200, 31)) )
    ...
    t_se      = pd.Series(t_model[:2], name='Time')
    params_d  = dict(zip(model.params, p))
    inputs_df = pd.DataFrame([i0, i0], columns=model.inputs)
    
    xo1, yo1 = obj_1(y_df, t_se, params_d, inputs_df)
