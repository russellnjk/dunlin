...
def save_best(index, best, collected, folder):
    filename = folder + f'/{index}_best.csv'
...
#Read data
data_filename       = 'data_MCr_scaled.csv'
dataset, scenarios_ = dp.read_dataset(data_filename, header=[0, 1, 2])
exp_data            = {'coarse_1': dataset}

#Synthetic exv data
for i in range(4):
    key                       = ('R_frac', len(scenarios_) +i, 'Time' )
    exp_data['coarse_1'][key] = np.array([0, 5e2]) 
exp_data['coarse_1'][('R_frac', 'Variance')] = 1e-3
...
#Save
save_figs('const', figs, t_figs, k_figs, folder=folder)
save_best('const', best, collected, folder=folder)
