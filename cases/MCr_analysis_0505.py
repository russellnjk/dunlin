...
df = pd.read_excel('data_MCr_20210505.xlsx', header=[0, 1, 2], sheet_name='All')

idx  = list(range(50))[::2] + list(range(50, len(df)))[::10]
time = df['Time']['Time']['Time']
data = df.iloc[idx]
t = time.values[idx]
data.index= t
...
colors = sim.colors['dull orange'], sim.colors['ocean'], sim.colors['cobalt']
colors = [c for c in colors for i in range(3)]
