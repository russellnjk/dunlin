...
def visualize(date):
    ...
    for n0, [i0, g0] in enumerate(df.groupby(by=col_lvl, axis=1)):
        ...
        lvl1    = sorted(lvl1, key=lambda x: x[0])[::-1]
        ...
        for n1, [i1, g1] in enumerate(lvl1):
            color = colors[n1] if '2C' not in i0 else colors2[n1]
            label = i1
...
if __name__ == '__main__':
    base_colors  = {'MCR'    : 'tomato red',
                   'Ind'     : 'goldenrod',
                   'C+Ind'   : 'cobalt',
                   '2C+Ind'  : 'sea'
                   }
    base_colors2 = {'MCR'    : 'purple',
                    'Ind'    : 'pink',
                    'C+Ind'  : 'electric blue',
                    '2C+Ind' : 'maroon'
                    }
    
    dates = ['210715', '210719', '210721', '210804']
    # dates = ['210812']
    dfs   = {}
    AXs   = {}
    ...
    #Other analysis
    dfs_     = {k:v for k,v in dfs.items() if '04' not in k}
    ...
    #Plot time response
    # base_colors  = {'M9+CA': {'MCR'    : 'tomato red',
    #                           'Ind'     : 'goldenrod',
    #                           'C+Ind'   : 'cobalt',
    #                           '2C+Ind'  : 'sea'
    #                           },
    ...
    # for i, g in mean.groupby(by=['Type', 'Inducer'], axis=1):
    #     base_color = 
    for n0, [i0, g0] in enumerate(mean.groupby(by=col_lvl, axis=1)):
        base_color  = base_colors[i0]
        base_color2 = base_colors2[i0]
    ...
    AX[0].set_title(f'OD600 Combined')
    AX[1].set_title('RFP/OD')
    AX[2].set_title('GFP/OD')
    AX[3].set_title('mu')
    AX[4].set_title('RFP/OD vs GFP/OD (st)')
