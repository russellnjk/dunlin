...
#Read the file
df   = pd.read_excel('data_210702_MCr.xlsx', index_col=0, header=[0, 1], sheet_name='Compiled')
...
#Plot for each media
colors = make_colors(len(M9CA['R_frac1'].columns), 'cobalt')
...
for color, label in zip(colors, labels):
    ...
    # AX[3].plot(t, M9CA['R_frac2'][label], '+', label=label, color=color)
    AX[4].plot(t, M9CA['GFP/OD'][label],  '+', label=label, color=color)
    
    
    start = 80
    stop  = 350
    x     = M9CA['mu'][label].loc[start+120:stop+120]
