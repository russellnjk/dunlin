...
def get_new_params(data_filename, model_filename, result_key):
    with open('cf_results.yaml', 'r') as file:
        yaml_data = ya.load(file, Loader=ya.FullLoader)
    
    yaml_data   = yaml_data[data_filename][result_key]
...
def protocol0(model_filename, data_filename, medium, result_key):
    ...
    new_params = get_new_params(data_filename, model_filename, result_key)
...
if __name__ == '__main__':
    ...
    fig, AX = dn.gridspec(4, 4, 
                          layout,
                          figsize=(8, 7.2), 
                          top=0.999, bottom=0.068, 
                          left=0.05, right=0.984,
                          wspace=0.45, hspace=0.8,
                          title=title
                          )
    
    img = mpimg.imread('figures/Model W.png')
    # pad = 15
    ...
    AX_ = AX[1:]
    model_filename   = 'curvefit_W1.dunl'
    data_filename    = 'curvefit_04Glu.csv'
    model, dataset_plot = protocol0(model_filename, data_filename, '0.4Glu', 'curvefit_Weisse_1.dun')
    make_prez_AX(AX_[0:4], model, dataset_plot, model_filename, data_filename)
    
    model_filename   = 'curvefit_W2.dunl'
    data_filename    = 'curvefit_04Glu.csv'
    model, dataset_plot = protocol0(model_filename, data_filename,  '0.4Glu', 'curvefit_Weisse_2.dun')
    ...
    for i, ax in zip('BCDEFGHIJKLMNOP', AX[1::]):
    ...
    fig.savefig('figures/W.png', dpi=1200)