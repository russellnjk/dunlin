...
if __name__ == '__main__':
    ...
    # temp = temp + sum(np.array([50, 1, 50, 10, 50]))
    # assert r == temp
    
    # def log_likelihood(params):
    #     return sum([abs(params[1] - 50), abs(params[3]-10)])
    ...
    # ###############################################################################
    # #Part 3: Optimization (Differential Evolution)
    # ###############################################################################
    ...
    # assert '_posterior' in a.columns

    # ###############################################################################
    # #Part 4A: Test Curvefitting
    # ###############################################################################
    # #Read model
    # dun_data, models = dn.read_file('optimization_test_files/differential_evolution1.dun')
    ...
    #                }
    # nominal     = {'p0': 0.1, 'p1': 0.1, 'u0': 1, 'u1': 9}
    # fig, AX     = dn.figure(1, 1)
    
    # #Pre-optimization check
    # OptResult = opt.OptResult(nominal, free_params, get_SSE)
    
    # r = OptResult.sampled_params[0].prior.pdf([1, 5, 11])
    # assert all( (r > 0) == [True, True, False])
    
    # r = OptResult.sampled_params[1].prior.pdf([1, 5, 11])
    # assert all( (r > 0) == [True, True, False])
    
    # r = get_SSE( np.array([0.1, 0.1, 5, 5]) )
    # assert r < 0.1
    
    # r = get_SSE( np.array([0.1, 0.1, 5, 6]) )
    # assert np.isclose(r, 3419, rtol=1e-2)

    # #Case 0: Manual instantiation
    # print('Case 0: Manual instantiation')
    # r = OptResult.differential_evolution(disp=True)
    
    # o = r['o']
    # assert all( np.isclose(o.x, [5, 5], rtol=1e-3) )
    # AX[0].plot(r['a']['_posterior'].values, label='Case 0')
    # AX[0].legend()
    
    # #Test accessing the results
    # assert np.all( OptResult['_posterior'] == r['a']['_posterior'] )
    
    # #Case 1: Test instantiation from model
    ...
    #                                     }
    #                     }
    # OptResults = opt.OptResult.from_model(model, to_minimize=get_SSE)
    # OptResult  = OptResults[0]
    
    # r = OptResult.differential_evolution()
    
    # o = r['o']
    # assert all( np.isclose(o.x, [5, 5], rtol=1e-3) )
    # AX[0].plot(r['a']['_posterior'].values, label='Case 1')
    # AX[0].legend()
    
    # #Case 2: Function-based optimization
    # print('Function-based optimization')
    # opt_results = opt.optimize_model(model, get_SSE, 'differential_evolution')
    # r           = opt_results[0]
    
    # o = r.o
    # assert all( np.isclose(o.x, [5, 5], rtol=1e-3) )
    # assert type(r.a) == pd.DataFrame
    # AX[0].plot(r['_posterior'].values, label='Case 2')
    # AX[0].legend()
    
    # #Case 3: Function-based parameter estimation
    ...
    # assert all( np.isclose(o.x, [5, 5], rtol=1e-3) )
    # assert type(r.a) == pd.DataFrame
    # AX[0].plot(r['_posterior'].values, label='Case 3')
    # AX[0].legend()
    
    # ###############################################################################
    # #Part 4B: Test EXV optimization
    # ###############################################################################
    ...
    # all_opt_results = opt.fit_models(models_, {'M1': dataset})
    # r               = all_opt_results['M1'][0]
    ...
    #Part 6: .dun File
    ###############################################################################
    dun_data, models = dn.read_file('optimization_test_files/differential_evolution2.dun')
    ...
    AX2_         = {('u0', 'u1'): AX2[0]}
    AX2_         = opt.plot_opt_results(opt_results, AX2_)
    ...
    opt_results = opt.fit_model(model, dataset, 'differential_evolution')
    AX_         = opt.plot_opt_results(opt_results, AX_, color='ocean')
    AX2_        = opt.plot_opt_results(opt_results, AX2_, color='ocean')
