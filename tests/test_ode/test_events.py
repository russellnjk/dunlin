...
if __name__ == '__main__':
    ...
    print('Case 1: Timer')
    #Expect: Spike at 209
    event0  = oev.Event(name='E0', execute=execute1, trigger_func=lambda t, *args: t-209)
    # event0.remove = True
    events  = [event0]
    t, y, p = ivp.integrate(func, tspan, y0, p0, events=events, overlap=True, include_events=True)
    ...
    print('Case 2: Event')
    #Expect: Spike at 25
    event0  = oev.Event(name='E0', trigger_func=trigger_func1, execute=execute1)
    events  = [event0]
    t, y, p = ivp.integrate(func, tspan, y0, p0, events=events, overlap=True, include_events=True)
    ...
    event0  = oev.Event(delay=400, persistent=True, name='E0', trigger_func=trigger_func1, execute=execute1)
    events  = [event0]
    t, y, p = ivp.integrate(func, tspan, y0, p0, events=events, overlap=True, include_events=True)
    ...
    #Expect: No spike
    event0  = oev.Event(delay=400, persistent=False, name='E0', trigger_func=trigger_func1, execute=execute1)
    events  = [event0]
    ...
    event0  = oev.Event(delay=10, persistent=True, name='E0', trigger_func=flipped, execute=execute1)
    events  = [event0]
    t, y, p = ivp.integrate(func, tspan, y0, p0, events=events)
    ...
    event0  = oev.Event(delay=0, name='E0', trigger_func=trigger_func1, execute=execute1)
    event1  = oev.Event(delay=0, name='E1', trigger_func=trigger_func2, execute=execute1)
    ...
    event0  = oev.Event(name='E0', execute=execute2, priority=0, trigger_func=lambda t, *args: t-800)
    event1  = oev.Event(name='E1', execute=execute1, priority=1, trigger_func=lambda t, *args: t-800)
    events  = [event0, event1]
    t, y, p = ivp.integrate(func, tspan, y0, p0, events=events, overlap=True, include_events=True)
    ...
    event0  = oev.Event(name='E0', trigger_func=lambda t, *args: t-200, execute=execute4)
    events  = [event0]
    t, y, p = ivp.integrate(func, tspan, y0, p, events=events, overlap=True, include_events=True)
    ...
    event0  = oev.Event(name='E0', trigger_func=trigger_func3, execute=execute3)
    events  = [event0]
    t, y, p = ivp.integrate(func, tspan, y0, p0, events=events, overlap=True, include_events=True)
    ...
    event0  = oev.Event(name='E0', execute=execute2, priority=0, trigger_func=lambda t, *args: t-900)
    event1  = oev.Event(name='E1', execute=execute1, priority=1, trigger_func=lambda t, *args: t-900)
    events  = [event0, event1]
    t, y, p = ivp.integrate(func, tspan, y0, p0, events=events, overlap=True, include_events=True)
    ...
    event0  = oev.Event(name='E0', execute=execute2, priority=0, trigger_func=lambda t, *args: t-455)
    event1  = oev.Event(name='E1', execute=execute1, priority=1, trigger_func=lambda t, *args: t-455)
    events  = [event0, event1]
    events  = [event0]
    t, y, p = ivp.integrate(func, tspan, y0, p0, events=events, overlap=True, include_events=False)
    ...
    events  = [event0, event1]
    events  = [event0]
    t, y, p = ivp.integrate(func, tspan, y0, p0, events=events, overlap=False, include_events=False)
    ...
    event1  = oev.Event(name='E1', execute=execute1, priority=1, trigger_func=lambda t, *args: t-455)
    events  = [event0, event1]
    t, y, p = ivp.integrate(func, tspan, y0, p0, events=events, overlap=False, include_events=True)
    ...
    ###############################################################################
    #Set up
    model_data = cmp.make_model_data(all_data, 'M1')
    ode        = odc.make_ode(model_data)
    event_objs = oev.make_events(ode, model_data)
    ...
    t, y, p = ivp.integrate(func_, tspan, y0, p0, events=events_, overlap=True, include_events=True)
